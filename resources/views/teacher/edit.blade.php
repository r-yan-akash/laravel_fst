...
<div class="container">
    ...
    <form action="{{route('teacher.update',$teacher->id)}}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input class="form-control" value="{{$teacher->teacher_id}}" name="teacher_id">
        </div>
        <div class="form-group">
            <label>Age</label>
            <input type="number " value="{{$teacher->age}}" class="form-control" name="age">
        </div>
        <div class="form-group">
            <label>Status</label>
            <select name="status" class="form-control">
                <option value="1" @if($teacher->status==1)selected="" @endif>Active</option>
                <option value="0" @if($teacher->status==0)selected="" @endif>Deactivate</option>
            </select>
        </div>
        <div class="form-group">
